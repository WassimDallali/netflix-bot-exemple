...
# Enhanced stealth script
    ...
    def get_fast_proxy(self, max_attempts: int = 3, country: str = "FR"):
        """
        Get a random SOCKS5 proxy (speed=1) from PremiSocks and validate HTTPS connectivity.
        Returns {'ip','port','proxy_string'} or None.
        """
        if not self.premsocks_token:
            return None
        headers = {"Authorization": f"Bearer {self.premsocks_token}"}
        url = f"https://premsocks.com/api/v1/socks/random?speed=1&country={country}"
        for attempt in range(1, max_attempts + 1):
            try:
                print(f"Info: Requesting fast proxy (attempt {attempt}/{max_attempts})...")
